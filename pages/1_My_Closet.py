...
if not rows:
    ...
    for row in rows:
        ...
        with st.container():
            ...
            with col1:
                st.markdown(f"**{item_name}** — {brand or 'No brand'} | {category} | Size: {size or 'N/A'} | Color: {color or 'N/A'} | Condition: {condition or 'N/A'} | Paid: {'$'+str(purchase_price) if purchase_price else 'N/A'} | Acquired: {acquired_date.strftime('%Y-%m-%d') if acquired_date else 'N/A'}")
