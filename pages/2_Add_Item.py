...
with st.expander("➕ Add New Item", expanded=False):
    with st.form("add_item_form"):
        ...
        acquired_date = st.date_input("Acquired Date")
        ...
        if submitted:
            ...
            if errors:
                ...
                try:
                    ...
                    cur.execute("""
                        INSERT INTO items (item_name, brand, category, size, color, condition, purchase_price, acquired_date, notes)
...
try:
    ...
    if rows:
        st.table([{"Name": r[0], "Brand": r[1], "Category": r[2], "Size": r[3], "Color": r[4], "Condition": r[5], "Price": f"${r[6]:,.2f}" if r[6] else "N/A", "Acquired": r[7].strftime("%Y-%m-%d") if r[7] else "N/A"} for r in rows])
    else:
