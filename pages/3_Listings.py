...
tab1, tab2 = st.tabs(["➕ Add New Listing", "📋 View Listings"])

# --- Tab 1: Add New Listing ---
with tab1:
    try:
        conn = get_connection()
        cur = conn.cursor()
...
with tab2:
    ...
    if not rows:
        ...
        st.table(
            [{"Name": r[1], "Brand": r[2], "Asking Price": f"${r[3]:,.2f}", "Platform": r[4] or "N/A", "Status": r[5], "Listed": r[6].strftime("%Y-%m-%d")} for r in rows]
        )
